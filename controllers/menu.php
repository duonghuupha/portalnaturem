...
class Menu extends Controller{
    ...
    function add(){
        ...
        if($typemenu == 1){// mot bai viet
            ...
            $link = '';
        }elseif($typemenu == 3){ // mot san pham
            $link = $_REQUEST['product_id'];
        }elseif($typemenu == 4){ // danh sach san pham
            $link = implode(",", $_REQUEST['product_cate']);
        }elseif($typemenu == 5){ // Lien he
            $link = '';
        }elseif($typemenu == 6){ // Lien ket ngoai
            $link = $_REQUEST['link'];
        }
        $data = array("code" => $code, "parent_id" => $parent_id, "title" => $title, "type_menu" => $typemenu,
                        "link" => $link, "position" => $position, "order_menu" => $order_menu, "active" => $active);
...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function list_pro(){
        ...
        $this->view->jsonObj = $jsonObj; //$this->view->perpage = $rows; $this->view->page = $get_pages;
        $this->view->render('menu/list_pro');
    }
    ...
    function list_pro_page(){
        ...
        $get_pages = isset($_REQUEST['page']) ? $_REQUEST['page'] : 1;
        $offset = ($get_pages-1)*$rows;
        $jsonObj = $this->model->get_data_pro_total($keyword);
        $this->view->total = $jsonObj; $this->view->perpage = $rows; $this->view->page = $get_pages;
        $this->view->render('menu/list_pro_page');
    }
}
?>
