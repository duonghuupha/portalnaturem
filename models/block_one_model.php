<?php
class Block_one_Model extends Model{
    ...
    function getFetObj($offset, $rows){
        ...
        $row = $query->fetchAll();
        $query = $this->db->query("SELECT id, code, title_1, title_2, image, status, create_at 
                                    FROM tbl_block_1 ORDER BY id DESC LIMIT $offset, $rows");
